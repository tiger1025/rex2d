...
/* 
	REX 2D ENGINE | Author: Tiger1025 | Site: http://tiger1025.com/rex/ 
*/
...
{ 
	if(rx.ellipse) //chrome
	{ 
		rx.save(); rx.beginPath(); rx.ellipse(x+w/2, y+h/2, w/2, h/2, 0, 0, Math.PI*2); rx.strokeStyle=style; rx.fill(); rx.restore(); 
	} else { // others
		rx.save(); rx.beginPath(); rx.translate(x+w/2, y+h/2); rx.scale(w/h, 1); rx.arc(0, 0, Math.min(w,h)/2, 0, Math.PI * 2, true);
  		rx.restore(); rx.closePath(); rx.fill(); 
  	}
}	
